...
<body>
    ...
    <?php require 'resources/views/nav.php'; ?>           
                   ...
                   echo "<form method='POST' action='datos-cliente.php?id=$row[id]&identificacion=$row[identificacion]&nombre=$row[nombre]'> 
                         <input type='hidden' name='id' value='$row[id]' />
                         <input type='hidden' name='identificacion' value='$row[identificacion]' />
                         <input type='hidden' name='nombre' value='$row[nombre]' />
                         <button type='submit' class='btn btn-primary col-sm-offset-10 '></span>Contactar</<button>
                    </form>";
                    echo "</div>
            </div>
        </div>";
